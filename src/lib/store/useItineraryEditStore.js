...
export const useItineraryEditStore = create((set, get) => ({
  ...
  addDay: () => set((state) => {
    ...
    return {
      modifiedSchedules: [
        ...
        { day: maxDay + 1, title: '', activities: [], transfers: [] },
      ],
    };
  }),

  removeDay: (dayIndex) => set((state) => {
  ...
  // Activity operations
  changeActivity: (dayIndex, activityIndex, newActivity) => set((state) => {
  ...
  // Transfer operations
  changeTransfer: (dayIndex, transferIndex, newTransfer) => set((state) => {
    const schedules = JSON.parse(JSON.stringify(state.modifiedSchedules));
  ...
  addTransfer: (dayIndex, transfer) => set((state) => {
    const schedules = JSON.parse(JSON.stringify(state.modifiedSchedules));
    schedules[dayIndex].transfers.push(transfer);
    return { modifiedSchedules: schedules };
  }),

  removeTransfer: (dayIndex, transferIndex) => set((state) => {
    const schedules = JSON.parse(JSON.stringify(state.modifiedSchedules));
