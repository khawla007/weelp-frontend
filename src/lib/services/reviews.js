 ...
 * @returns {Promise<Object>} Reviews data with summary
 */
export async function getItineraryReviews(itinerarySlug, params = {}) {
  try {
    const { sort = 'recent', photos_only = false, per_page = 10, page = 1 } = params;
    ...
    const queryString = queryParams.toString();
    const response = await publicApi.get(`/api/reviews/itinerary/${itinerarySlug}${queryString ? `?${queryString}` : ''}`, {
      headers: { Accept: 'application/json' },
    });
...
/**
 * Get featured reviews for a specific itinerary
 * Used on: Single Itinerary Page - featured review carousel
 * @param {string} itinerarySlug - Itinerary slug
 * @returns {Promise<Object>} Featured reviews data
 */
export async function getItineraryFeaturedReviews(itinerarySlug) {
  try {
    const response = await publicApi.get(`/api/reviews/itinerary/${itinerarySlug}/featured`, {
      headers: { Accept: 'application/json' },
    });
