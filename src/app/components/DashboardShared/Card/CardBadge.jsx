...
export function CardBadge({
  ...
  if (type === 'featured') {
    return (
      <>
        <div className={`absolute top-4 left-4 z-10 bg-[#568f7c] text-white text-xs px-2 py-1 rounded-md font-medium ${className}`}>
          Featured
        </div>
        <Star
          size={24}
          fill="#568f7c"
          strokeWidth={2}
          className="absolute top-4 right-4 z-10 text-[#568f7c] drop-shadow-[0_2px_4px_rgba(86,143,124,0.3)]"
        />
      </>
    );
  }
  ...
  if (type === 'location') {
    return (
      <Badge className={`bg-[#568f7c] text-white hover:bg-[#4a7a6a] ${className}`}>
        {label}
      </Badge>
    );
  }
