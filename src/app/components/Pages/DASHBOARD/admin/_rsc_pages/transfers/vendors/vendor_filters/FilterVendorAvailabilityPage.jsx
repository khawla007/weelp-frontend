...
import { CardVendorAvailability } from '../shared/vendor_cards';
import useSWR from 'swr';
import { useParams } from 'next/navigation';
import { fetcher } from '@/lib/fetchers';
import { VendorNoResultFound } from '../shared/VendorNoResultFound';

const FilterVendorAvailabilityPage = () => {
        ...
        <form className="space-y-4">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Input type="search" placeholder="Search availability..." {...field} className="max-w-md" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
      ...
      {availability?.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {availability.map((available, index) => (
            <CardVendorAvailability key={index} {...available} />
          ))}
        </div>
      )}
...
export default FilterVendorAvailabilityPage;
