...
import { createCountry, editCountry } from '@/lib/actions/country';
...
const CreateCountryForm = ({ apiFormData = {} }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let res;
      if (id) {
        res = await editCountry(id, mergedData); // edit action
      } else {
        res = await createCountry(mergedData); // create action
      }
      ...
      if (res.success) {
        ...
        router.push('/dashboard/admin/destinations/countries/'); // back push to countries
      } else {
        toast({
          title: 'Error',
  ...
  if (isClient) {
    return (
      <div className="min-h-screen w-full bg-muted py-4 sm:px-6 lg:px-8">
        <NavigationDestinations title={id ? 'Edit country' : 'New Country'} description={`${id ? ' Edit' : ' Create a new'} country destination with detailed information`} />
