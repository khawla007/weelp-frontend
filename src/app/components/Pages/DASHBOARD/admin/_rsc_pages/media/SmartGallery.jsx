      ...
      {images.map((file, index) => (
        <div key={index} className="relative size-24 group border rounded-md">
          ...
          <button type="button" onClick={() => removeImage(index)}>
            <Trash2 className="absolute  hidden group-hover:block top-3/4 left-3/4 p-1 -m-2 bg-background text-foreground border-none rounded-md  size-6 cursor-pointer" size={15} />
          </button>
        </div>
      ))}
    </div>
