...
export const EditItineraryForm = ({ categories, attributes, tags, locations = [], allactivities, alltransfers, itineraryData, isCreatorItinerary = false }) => {
  ...
  const ScheduleTab = () => {
    ...
    // Live preview — unit base prices only. Per-person multiplication and
    // discounts apply at checkout, not in the itinerary view.
    ...
    return (
      <div className="pb-4 relative">
        ...
        <div className="mt-3 mb-4 rounded-md border border-dashed border-border px-4 py-2 text-sm space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Activities (base prices)</span>
            <span className="font-semibold text-foreground">{previewBreakdown.activities.toFixed(2)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Transfers (base + extras)</span>
            <span className="font-semibold text-foreground">{previewBreakdown.transfers.toFixed(2)}</span>
          </div>
          <div className="text-[12px] text-muted-foreground">Base subtotal. Per-person multiplication and discounts apply at checkout.</div>
        </div>
            ...
            (
              ...
              <div key={item?.id} className="space-y-4">
                <div className="flex items-center gap-4 mt-4 justify-between">
                  ...
                  <Trash2 onClick={() => setPendingRemoval({ kind: 'day', payload: { item, itineraryId: id, dayIndex: index } })} className="text-destructive cursor-pointer" size={16} />
                </div>
                          ...
                          <div key={activityIndex} className="p-4 flex w-full border rounded-md items-center gap-4">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2 flex-wrap items-center">
                                ...
                                <Trash2 onClick={() => setPendingRemoval({ kind: 'activity', payload: { filteredActivity, itineraryId: id } })} className="text-destructive cursor-pointer" size={16} />
                              </div>
                            </div>
                          </div>
                        );
                            ...
                            <div key={transferIndex} className="p-4 flex w-full border rounded-md items-center gap-4">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex gap-2 flex-wrap items-center">
                                  ...
                                  />
                                  <Trash2
                                    onClick={() => setPendingRemoval({ kind: 'transfer', payload: { filteredTransfer, itineraryId: id } })}
    ...
    <div className="min-h-screen w-full bg-muted py-12 sm:px-6 lg:px-8">
      ...
      <div className="w-full space-y-4">
        <FormProvider {...methods}>
          ...
          >
            <fieldset className={`${currentStep === 3 ? '' : 'bg-background p-8 border shadow rounded-lg'} ${(isSubmitting || isApproving) && ' cursor-wait'}`} disabled={isSubmitting || isApproving}>
