...
const FilterItinerary = ({ categories = [], difficulties = [], durations = [] }) => {
  ...
  const handleMultpleDelete = async () => {
    try {
      const res = await deleteMultipleItineraries(selectedItems);
      if (res.success) {
  ...
  return (
    <Card className="flex gap-4 flex-col lg:flex-row ">
      {/* Sidebar Filter */}
      <ListingFilterSidebar control={control} filters={filters} categories={categories} difficulties={difficulties} durations={durations} searchPlaceholder="Search Itinerary" />
