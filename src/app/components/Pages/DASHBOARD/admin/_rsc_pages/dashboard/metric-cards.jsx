  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      {cards.map((item, index) => {
        ...
        return <StatCard key={index} label={item.title} icon={<IconComponent size={14} className="text-[#18181b] text-sm font-medium" />} value={formattedValue} change={item.change} />;
      })}
    </div>
  );
};
