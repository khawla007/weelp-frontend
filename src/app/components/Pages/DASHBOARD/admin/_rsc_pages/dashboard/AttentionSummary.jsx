...
function SignalLink({ href, className, children }) {
  return (
    <NavigationLink href={href} className={`inline-flex items-center gap-1 font-medium ${className}`}>
    ...
    <section aria-labelledby="attention-summary-title" className="min-w-0">
      <div className="h-full rounded-[15px] border border-border bg-card p-[15px] text-card-foreground">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <NavigationLink href="/dashboard/admin/orders" className="text-xs font-medium text-muted-foreground hover:text-foreground">
            View all →
          </NavigationLink>
        </div>
        {isLoading ? (
