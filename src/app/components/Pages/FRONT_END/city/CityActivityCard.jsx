...
export default function CityActivityCard({ href, image, title, rating, reviewCount, price, discount, className = '' }) {
  return (
    <div
    ...
    >
      ...
      <Link href={href} className="relative overflow-hidden block">
        <img src={image} alt={title} className="h-[200px] w-full object-cover transition duration-500 group-hover:scale-105" />
      </Link>
      ...
      <div className="flex flex-1 flex-col gap-[14px] px-4 pb-4 pt-3">
        ...
        {/* Title */}
        <Link href={href}>
          <h3 className="text-[17px] leading-[1.4] text-[#142a38] line-clamp-2" style={{ fontFamily: 'var(--font-interTight), Inter Tight, sans-serif', fontWeight: 700 }}>
            {title}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between mt-auto">
          {price && (
            <span className="text-[16px] leading-[1.37] text-[#1a2e3d]" style={{ fontFamily: 'var(--font-inter), Inter, sans-serif', fontWeight: 600 }}>
              From {price}
            </span>
          )}
          {discount && (
        ...
        <Link
          href={href}
          className="weelp-city-cta-button flex h-[40px] w-full items-center justify-center rounded-[10px] transition hover:opacity-95"
