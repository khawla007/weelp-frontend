...
export default function FilterSidebar({
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => { if (res.status === 200) setCategories(res?.data?.data || []); })
      .catch(() => setCategories([]));
  }, []);
  ...
  return (
    <div
      className="w-full sm:max-w-xs bg-white h-fit rounded-[11.5px] p-6 px-7"
      style={{ boxShadow: '0 2.22px 5.63px rgba(0,0,0,0.05)' }}
    >
      <div className="flex flex-col gap-10">
        {/* Categories */}
        <div>
          <h3 className="text-[18px] text-[#143042] mb-4" style={{ ...FONT, fontWeight: 500 }}>Categories</h3>
          <div className="flex flex-col space-y-3 max-h-48 overflow-auto">
            <label className={`flex items-center gap-2 cursor-pointer ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              />
              <span className="text-[18px] text-[#435a67]" style={{ ...FONT, fontWeight: 500 }}>All</span>
            </label>
            {categories.map((cat, i) => (
              <label key={i} className={`flex items-center gap-2 cursor-pointer ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
                <input
                  ...
                  className="size-[19px] rounded-[2px] border-2 border-[#667085] accent-[#57947d]"
                />
                <span className="text-[18px] text-[#435a67]" style={{ ...FONT, fontWeight: 500 }}>{cat?.name}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        {/* Price Range */}
        <div>
          <h3 className="text-[18px] text-[#143042] mb-4" style={{ ...FONT, fontWeight: 500 }}>Price</h3>
          <div className={disabled ? 'opacity-50 pointer-events-none' : ''}>
            <ReactRangeSliderInput
              ...
              className="w-full city-price-slider"
              disabled={disabled}
            />
            <div className="flex justify-between mt-2">
              <span className="text-[14px] text-[#435a67]" style={{ ...FONT, fontWeight: 500 }}>${priceRange[0]}</span>
              <span className="text-[14px] text-[#435a67]" style={{ ...FONT, fontWeight: 500 }}>${priceRange[1]}</span>
            </div>
          </div>
        </div>
        ...
        {/* Ratings */}
        <div>
          <h3 className="text-[18px] text-[#143042] mb-4" style={{ ...FONT, fontWeight: 500 }}>Ratings</h3>
                ...
                <div className="flex">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      size={19}
                      className={i < r ? 'fill-[#fed141] stroke-none' : 'stroke-[#fed141] fill-none'}
                      strokeWidth={2}
                    />
                  ))}
                </div>
        ...
        <div>
          <h3 className="text-[18px] text-[#143042] mb-4" style={{ ...FONT, fontWeight: 500 }}>Location</h3>
          <div className="flex flex-col space-y-3">
            {['Asia', 'Europe', 'Middle East', 'America'].map((loc) => (
              <label key={loc} className="flex items-center gap-2 cursor-pointer opacity-50">
                ...
                <span className="text-[18px] text-[#435a67]" style={{ ...FONT, fontWeight: 500 }}>{loc}</span>
              </label>
            ))}
          </div>
        </div>
