import { Card } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import React from 'react';

export const LoadingPage = () => {
  return (
    <div className="h-screen w-screen flex items-center justify-center bg-background bg-opacity-50">
      <PageSkeleton />
    </div>
  );
};

/**
 * FieldSkeleton - Input-height placeholder for a form field / dropdown while its
 * options load. The label row is OFF by default, because most callsites keep their
 * title/label always visible and only swap the control (WidgetCard cardTitle in
 * BlogSidebar, AccordionTrigger in FilterBlogPage, the shared add-on tabs,
 * DynamicSite) — reserving a label there would double-stack under the real title
 * and cause a layout jump.
 *
 * Pass `withLabel` only where the real swapped-in content includes its own
 * FormLabel inside the conditionally-rendered block (e.g. ReviewForm's user/item
 * FormItems), so the skeleton reserves the label row to match.
 *
 * @param {boolean} withLabel - Render a label-row placeholder above the field (default false).
 */
export const FieldSkeleton = ({ withLabel = false, className = '' }) => {
  return (
    <div className={`space-y-2 ${className}`} aria-hidden="true">
      {withLabel && <Skeleton className="h-4 w-24" />}
      <Skeleton className="h-10 w-full rounded-md" />
    </div>
  );
};

/**
 * FormSkeleton - Stacked label+field rows plus a submit bar, shaped like a typical
 * edit form. Shown while a form-edit route loads its record.
 *
 * @param {number} fields - How many label+field rows to render (default 5).
 */
export const FormSkeleton = ({ fields = 5, className = '' }) => {
  return (
    <div className={`w-full max-w-2xl mx-auto space-y-6 p-4 ${className}`} aria-hidden="true">
      <Skeleton className="h-8 w-1/3" />
      {Array.from({ length: fields }).map((_, index) => (
        <div key={index} className="space-y-2">
          <Skeleton className="h-4 w-24" />
          <Skeleton className="h-10 w-full rounded-md" />
        </div>
      ))}
      <Skeleton className="h-10 w-32 rounded-md" />
    </div>
  );
};

/**
 * PageSkeleton - Generic page placeholder. Used for full-screen route loads and
 * Suspense fallbacks where there's no page-specific shape (SingleVendorPage with a
 * md:grid-cols-2 card body, PaymentSuccessClient with an ~85vh lg:grid-cols-3 layout
 * plus a long table).
 *
 * Since one shared skeleton can't pixel-match both, it aims for generic fidelity
 * rather than overfitting either page: a tall min-height so it doesn't collapse far
 * below a full-screen page, a title + subtitle row, a responsive two-column grid of
 * card-shaped blocks (echoing the side-by-side card bodies), and a stack of row
 * placeholders below (echoing the success-page table).
 */
export const PageSkeleton = ({ className = '' }) => {
  return (
    <Card className={`w-full max-w-4xl mx-auto min-h-[60vh] space-y-6 p-6 ${className}`} aria-hidden="true">
      {/* Title + subtitle */}
      <div className="space-y-2">
        <Skeleton className="h-8 w-1/2" />
        <Skeleton className="h-4 w-2/3" />
      </div>

      {/* Two card-shaped blocks side by side (stack on mobile) */}
      <div className="grid md:grid-cols-2 gap-4">
        <Skeleton className="h-40 w-full rounded-lg" />
        <Skeleton className="h-40 w-full rounded-lg" />
      </div>

      {/* Stacked rows echoing a detail table */}
      <div className="space-y-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <div key={index} className="flex items-center justify-between gap-4">
            <Skeleton className="h-4 w-1/3" />
            <Skeleton className="h-4 w-1/4" />
          </div>
        ))}
      </div>
    </Card>
  );
};

/**
 * MediaGridSkeleton - Square-aspect card grid matching the media library layout
 * (grid-cols-2 → xl:grid-cols-5). Shown while media images load/paginate.
 *
 * @param {number} count - How many placeholder tiles to render (default 10).
 */
export const MediaGridSkeleton = ({ count = 10 }) => {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4" aria-hidden="true">
      {Array.from({ length: count }).map((_, index) => (
        <Skeleton key={index} className="aspect-square w-full rounded-lg" />
      ))}
    </div>
  );
};

export const ProductCardSkelton = ({ className }) => {
  return (
    <div role="status" className={`${className ?? ''} sm:max-w-xs w-full p-4 border border-border rounded shadow animate-pulse dark:border-border`}>
      <div className="flex items-center justify-center h-48 mb-4 bg-muted rounded dark:bg-muted"></div>
      <div className="h-2.5 bg-muted rounded-full dark:bg-muted w-48 mb-4"></div>
      <div className="h-2 bg-muted rounded-full dark:bg-muted mb-2.5"></div>
      <div className="h-2 bg-muted rounded-full dark:bg-muted mb-2.5"></div>
      <div className="h-2 bg-muted rounded-full dark:bg-muted"></div>
      <div className="flex items-center mt-4 gap-2">
        <button className="bg-muted rounded-sm dark:bg-muted w-48 py-4  h-4"></button>
        <div>
          <div className="h-2.5 bg-muted rounded-full dark:bg-muted w-32 mb-2"></div>
          <div className="w-48 h-2 bg-muted rounded-full dark:bg-muted"></div>
        </div>
      </div>
    </div>
  );
};

export const DestinationCardSkelton = ({ imgHeight, imgWidth, className }) => {
  return (
    <div className={`${className} relative w-full sm:w-fit mx-auto animate-pulse`}>
      {/* Skeleton for Image */}
      <div className={`bg-muted object-cover w-full ${imgHeight ?? 'h-[360px]'} ${imgWidth ?? 'sm:w-[220px] lg:w-60'} rounded-lg`}></div>

      {/* Skeleton for Text Overlay */}
      <div className="absolute bottom-0 w-full p-6 rounded-b-lg bg-muted">
        {/* Skeleton for Title */}
        <div className="h-6 bg-muted rounded w-3/4 mb-2"></div>
        {/* Skeleton for Subtitle */}
        <div className="h-4 bg-muted rounded w-1/2"></div>
      </div>
    </div>
  );
};

export const TestimonialCardSkelton = ({ className }) => {
  return (
    <div className={`${className ?? 'flex'}  flex-col gap-4 max-w-sm w-full  rounded-lg p-4 py-6  border-border shadow-md h-full animate-pulse`}>
      <div className="flex gap-4 items-center flex-wrap">
        <div className={`rounded-full w-16 h-16 bg-muted border-border animate-pulse`} />

        <div>
          <h3 className={`text-foreground text-xl font-semibold flex items-center gap-2 animate-pulse`}>
            <span className={`w-24 bg-muted h-4 animate-pulse`} /> {/* Placeholder for name */}
            <span className={`w-6 h-6 rounded-full  animate-pulse`} /> {/* Placeholder for badge */}
          </h3>

          <span className={`font-normal text-muted-foreground w-32 h-4 bg-muted animate-pulse`} />
        </div>

        <button className={`border border-border p-2 px-6 rounded-3xl shadow-md w-fit text-muted-foreground font-normal animate-pulse`}>
          <span className={`w-20 bg-muted h-4 animate-pulse`} />
        </button>
      </div>

      <div>
        <h3 className={`text-foreground text-xl mb-3 w-full bg-muted h-6 animate-pulse`} />
      </div>

      <div className={`flex justify-between flex-wrap`}>
        <span className={`text-muted-foreground font-normal text-base uppercase w-32 bg-muted h-4 animate-pulse`} />
      </div>
    </div>
  );
};

export const ReviewCardSkelton = ({ className }) => {
  return (
    <div className={`${className ?? ''}  max-w-full sm:max-w-xs w-full `}>
      <div className="w-full p-4 bg-background shadow-lg rounded-lg animate-pulse transform transition duration-1000">
        <div className="flex items-center mb-16">
          <div className="h-5 w-24 bg-muted rounded-md"></div>
          <div className="ml-auto flex space-x-2">
            <div className="w-5 h-5 bg-muted rounded-full"></div>
            <div className="w-5 h-5 bg-muted rounded-full"></div>
            <div className="w-5 h-5 bg-muted rounded-full"></div>
          </div>
        </div>
        <div className="bg-muted h-4 w-full rounded-md mb-2"></div>
        <div className="bg-muted h-4 w-3/4 rounded-md mb-2"></div>
        <div className="bg-muted h-4 w-2/4 rounded-md mb-2"></div>
      </div>
    </div>
  );
};

export const ImageSkeltonCard = ({ className }) => {
  return <div className={`${className} skeleton-image max-w-xs animate-pulse bg-muted w-full h-72 rounded-md mb-4`} />;
};

export const DashboardCardSkelton = ({}) => {
  return (
    <Card className="w-full lg:w-fit border rounded-lg">
      <div className="w-full lg:w-[326px] h-[183px] bg-muted animate-pulse rounded-lg" />
      <div className="p-4 space-y-2">
        <div className="flex gap-4">
          <div className="w-5 h-5 bg-muted rounded-full"></div>
          <div className="w-5 h-5 bg-muted rounded-full"></div>
          <div className="w-5 h-5 bg-muted rounded-full"></div>
        </div>

        <div className="flex gap-2 flex-col">
          <div className="w-1/2 h-5 bg-muted rounded-full"></div>
          <div className="w-1/2 h-5 bg-muted rounded-full"></div>
        </div>
      </div>
    </Card>
  );
};
